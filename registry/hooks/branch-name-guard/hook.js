...
const DEFAULT_PATTERN = /^(feature|bug|fix|hotfix|docs|chore|test|security|refactor|perf|ci)\/.+/;
// Matches: git checkout -b <name>, git switch -c <name>
const BRANCH_CREATE_RE = /\bgit\s+(?:checkout\s+-b|switch\s+-c)\s+(['"]?)(\S+)\1/;
function extractBranchName(command) {
    const match = BRANCH_CREATE_RE.exec(command);
    return match ? match[2] : null;
}
function getBranchPattern() {
    ...
    if (envPattern) {
        try {
            ...
            process.stderr.write(`branch-name-guard: invalid BRANCH_NAME_PATTERN — falling back to default\n`);
        }
    }
...
async function main() {
    ...
    if (!branchName)
        process.exit(0); // not a branch creation command
    const pattern = getBranchPattern();
    if (pattern.test(branchName))
        process.exit(0); // name is valid
    ...
    process.stderr.write(`branch-name-guard: blocked — branch name does not follow conventions\n` +
        `  Branch: ${branchName}\n` +
        `\n` +
